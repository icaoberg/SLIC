function [nuclei_mask] = ml_seeding(dna,resolution, seeding)
...
% Author: Ivan Cao-Berg (August 4, 2005)
% Yanhua Hu Using standard ml functions.
% Estelle Glory 11-2007 Modified by  to split the segmentation into 4 steps

% Copyright (C) 2005 Murphy Lab
...
if seeding.withWatershed==1
    imComplement = uint8(imcomplement(nuclei_mask));
    distImg =  uint8(bwdist(imComplement));
    imComplement2 =  uint8(imcomplement(distImg));
    cellImg = watershed(imComplement2);
    nuclei_mask(find(cellImg==0))=0;


%     figure, imshow(imComplement,[]), title('complement');
