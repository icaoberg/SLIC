function [lines, masks] = ml_segment_cells_voronoi(im, dnaIm)
...
nuclei_obj = filter_nuclei2(dnaobj, size(dnaBin), dnaBin);
...
[totalCofAll, objCofsAll] = ml_findCOFs(dnaobj, dna);
%keyboard;
%%%%%%%%%%%%%%%%%%%% END %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while(~isempty(objsToDo))
    ...
    keyboard;
    singleObj = zeros(size(im));
    singleObj(find(im == 1)) = 1;
    singleObj(find(tempIm==3)) = 1;
    h = ml_3dfindobj(uint8(singleObj), 1);
...
function objs = split_obj(obj, im_size)
singleObj = zeros(im_size);
keyboard;
singleObj(find(im2 == 1)) = 1;
%while (round(tempSize*100/ar) > 0)
%    singleObj = mmdist(singleObj);
