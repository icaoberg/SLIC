...
% can specify your own directory instead of default temporary 
% directory. If no input argument, assume defatult temporary
% obtained from TEMPDIR function.  
...
% Updated method to reflect changes in ml_excludefile.m
...
len = length(dirname);
if( dirname(len) ~= filesep) dirname(len+1) = filesep; end;
...
while( 1)
  ...
  answer = ml_createfile_excl( FullName);
  if answer
        if( nargout < 2)
            arg1 = FileName;
        else
